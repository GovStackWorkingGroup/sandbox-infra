...
resource "aws_lb_target_group" "mifos_bb_api" {
  name = "mifos-bb-api-tg"
  port = "5000"
  vpc_id = var.vpc_id
  protocol = "HTTP"
  target_type = "ip"
...
resource "kubernetes_manifest" "mifos_bb_api" {
  lifecycle {
    replace_triggered_by = [aws_lb_target_group.mifos_bb_api]
  }
  manifest = {
    apiVersion = "elbv2.k8s.aws/v1beta1"
    kind       = "TargetGroupBinding"
    metadata = {
      name      = aws_lb_target_group.mifos_bb_api.name
    ...
    spec = {
      ...
      targetGroupARN = aws_lb_target_group.mifos_bb_api.arn
    }
  }
}
...
resource "aws_lb_listener_rule" "mifos_bb_admin_api" {
  lifecycle {
    replace_triggered_by = [aws_lb_target_group.mifos_bb_api]
  }
  listener_arn = var.sandbox_alb_listener_arn
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.mifos_bb_api.arn
  }
